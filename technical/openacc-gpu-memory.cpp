/*
Filename: openacc-gpu-memory.cpp
Author: Ralph Cajipe
Date: March 18, 2023
Description: This program adds up the elements in an array using OpenACC with GPU and GPU Memory.
The array is initialized with a repeating pattern of 0's and 1's, and the sum of the array is
calculated using a function that iterates through the array with OpenACC parallelization and GPU memory.

Functions:
    int array_sum(int* arr, int size): calculates the sum of an array with OpenACC parallelization and GPU memory
...
int main() {
    ...
    // Measure the time taken for OpenACC with GPU and GPU Memory execution
